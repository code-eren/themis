...
contract Campaign {
    ...
    constructor(uint256 _matchId, uint256 _teamId0, uint256 _teamId1, uint256 _initialOdds0, uint256 _initialOdds1){
        owner = msg.sender;
        matchId = _matchId;
        teamId0 = _teamId0;
        teamId1 = _teamId1;
    ...
    function bid(uint256 _teamId) external payable {
        require(_teamId == teamId0 || _teamId == teamId1, "can only bid for team involved in this match");
    ...
    function claim() external {
        ...
        require(fulfilled, "can't claim before result is fulfilled from oracle");
        uint256 amount = 0;
        for(uint i = 0; i < addr2bidder[sender].bids.length; i++){
        ...
        require(address(this).balance >= amount, "not enough balance in the contract to make the transfer");
        if(amount > 0){
