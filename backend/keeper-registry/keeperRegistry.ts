...
class KeeperRegistry {
  ...
  linkabi = ["function transferAndCall(address to, uint256 value, bytes memory data) external returns (bool)"];
  ...
  async register(name, encryptedEmail, upkeepContract, gasLimit, adminAddress, checkData, amount, source){
   ...
   let tx = await this.linkcontract.transferAndCall(this.registryAddr, ethers.utils.parseUnits("20", 18), calldata);
   console.log(tx);
  }
